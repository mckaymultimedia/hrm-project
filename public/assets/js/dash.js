...
document.addEventListener("DOMContentLoaded", function () { 
  ...
  function showModal(users) {
    ...
    modalContainer.classList.add('custom-modal', 'performanceModal');
   const modalContent = `
      <div class="custom-modal-content performanceModal-content">
        <span class="custom-close" id="close_duration_period">&times;</span>
    ...
    const modalText = modalContainer.querySelector('#modalText');

    const userIds = [];

    users.forEach(user => {
      const userContent = `
        <p style="font-size:19px;border-bottom:1px solid gray; padding-bottom:10px;"><strong class="probation_name" style="color:green">${user.name}</strong> has completed ${user.period} in the company.</p>
      `;
      modalText.insertAdjacentHTML('beforeend', userContent);
      userIds.push({ id: user.id, period: user.period });

    });

    const closeButton = modalContainer.querySelector('.custom-close');

    modalContainer.style.display = 'block';

    closeButton.addEventListener('click', function() {
      updateStatus(userIds ,'closed');
  ...
  // Function to schedule data fetching
  ...
  // Set interval to continuously call scheduleFetch every hour
  setInterval(scheduleFetch, 3600000); // Check every hour
});
...
function checkAndShowModal() {
  ...
  const modal = localStorage.getItem('monthly_modal');
  
  if (currentDay >= 27 && currentDay <= 30) {
  fetch('/performance-reviews/team')
      .then(response => {
          if (!response.ok) {
              throw new Error('Network response was not ok');
          }
          return response.text().then(text => JSON.parse(text.replace(/^.*?({.*}).*?$/, '$1')));

      })
      .then(data => {
          console.log('Response data:', data);
          if (data && data.teamLeadExists) {
            if(modal !== 'closed'){
              showModal("Please fill your intern performance monthly form.");
            }
          } else {
              console.log('No users found');
          }
  ...
  localStorage.removeItem('monthly_modal');
...
function showModal(message) {
  ...
  const modalContainer = document.createElement('div');
  modalContainer.classList.add('custom-modals');

  // Create modal content
  const modalContent = `
      <div class="modal-content monthly_performance">
     
        <span style="width:100%;display:flex;justify-content:end; border-bottom:1px solid gray;" class="close" id="monthly_perf">&times;</span>
  ...
  closeButton.addEventListener('click', function () {
    localStorage.setItem('monthly_modal', 'closed');
      modalContainer.style.display = 'none';
  });
