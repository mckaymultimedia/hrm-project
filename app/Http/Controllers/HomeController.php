...
class HomeController extends Controller
{
    ...
    {
        $this->CheckEmployeeReviewStatus();
    }
        ...
        if (Auth::check()) {
            ...
            if ($user->type == 'employee') {
                ...
                foreach ($employees as $employee) {
                    $db = $employee->company_doj;
                    $doj = Carbon::parse($db);
                    $doj->year(date('Y'));

                    $diff = Carbon::now()->diffInDays($doj, false);
                    if ($diff >= 0 && $diff <= 7) {
                        $work_anniversaries[] = $employee;
                    }
                }
                $empDOB = [];
                ...
                return view('dashboard.dashboard', compact('arrEvents', 'announcements', 'employees', 'meetings', 'employeeAttendance', 'officeTime', 'work_anniversaries', 'empDOB'));
            } else {
                $employees = Employee::all();
                $work_anniversaries = [];
                ...
                $performanceReview = PerformanceReview::select('employee_id')->where('status', 'pending')->get();


                return view('dashboard.dashboard', compact('performanceReview', 'empDOB', 'work_anniversaries', 'arrEvents', 'announcements', 'activeJob', 'inActiveJOb', 'meetings', 'countEmployee', 'countUser', 'countTicket', 'countOpenTicket', 'countCloseTicket', 'notClockIns', 'countEmployee', 'accountBalance', 'totalPayee', 'totalPayer'));
            }
        } else {
    ...
    private function CheckEmployeeReviewStatus()
    {
        $employees = Employee::all();
        ...
        foreach ($employees as $employee) {
            ...
            if ($latestReview) {
                // Extract month and day components from review_date
                list($reviewMonth, $reviewDay) = explode('-', Carbon::parse($latestReview->review_date)->format('m-d'));
                // Extract month and day components from current date
                list($currentMonth, $currentDay) = explode('-', Carbon::now()->format('m-d'));

                $differenceInDays = Carbon::create(null, $reviewMonth, $reviewDay)
                    ->diffInDays(Carbon::create(null, $currentMonth, $currentDay));
                // dd($differenceInDays);
