                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        @php
                                            $employees = \App\Models\User::where('created_by', \Auth::user()->creatorId())->get();
                                        @endphp
                                        <select class="form-select col-md-1" name="team_lead"
                                            aria-label="Default select example">
                                            <option value="">Select TeamLead</option>
                                            @foreach ($employees as $e)
                                                <option value="{{ $e->id }}" @if ($employee->team_lead == $e->id) selected @endif>{{ $e->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    {{-- <div class="form-group col-md-6">
                                    ...
                                    <div class="form-group mt-2">
                                        ...
                                        {{ Form::date('company_doj', null, ['class' => 'form-control current_date', 'required' => 'required', 'autocomplete' => 'off', 'placeholder' => 'Select Company Date Of Joining', 'id' => 'company_doj']) }}
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label">{{ __('Probation Completion Date') }}</label>
                                        <input type="date" class="form-control" name="probation_date" id="probation_date"
                                            value="{{$employee->pc_date}}">
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('script-page')
    ...
    <script>
        $(document).ready(function() {
            $('#company_doj').on('change', function() {
                var selectedDate = new Date($(this).val());
                var probationDate = new Date(selectedDate.getTime() + (90 * 24 * 60 * 60 *
                    1000));
                var formattedProbationDate = probationDate.toISOString().split('T')[0];
                $('#probation_date').val(formattedProbationDate);
            });
        });
    </script>
