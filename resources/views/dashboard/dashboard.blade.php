                ...
                <div class="col-xl-5">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body" style="height: 324px; overflow:auto">
                            @foreach ($work_anniversaries as $employee)
                                <div class="card"
                                    style="padding: 12px;
                                min-height: 60px;
                                max-height: 125px;
                              ...
                              ">
                                    ...
                                    @php
                                        $company_doj = \Carbon\Carbon::createFromFormat('Y-m-d', $employee->company_doj);
                                        $formatted_date = $company_doj->format('l, F j');
                                        $current_year = \Carbon\Carbon::now()->year;
                                    @endphp

                                    <p><b>On</b>: {{ $formatted_date }}@if ($company_doj->year === $current_year)
                                            , {{ $company_doj->year }}
                                        @endif
                                    </p>
                    ...
                    <div class="card">
                        <div class="card-header card-body table-border-style">
                            <h5>Pending Performance Reviews</h5>
                        </div>
                        <div class="card-body" style="height: 324px; overflow:auto">
                            @if ($performanceReview->count() > 0)
                                @foreach ($performanceReview as $employee)
                                    <div class="card"
                                        style="padding: 12px;
                                min-height: 60px;
                                max-height: 125px;
                              ...
                              ">
                                        ...
                                        @php
                                            $employeeData = App\Models\Employee::where('id', $employee->employee_id)->first();
                                        @endphp
                                        <h4>{{ $employeeData->name }}</h4>
                                        <p>({{ $employeeData->designation->name }})</p>
                                        <a href="#"
                                            data-url="{{ route('performance.createHome', $employeeData->id) }}"
                                            data-ajax-popup="true" data-size="lg"
                                            data-title="{{ __('Performance Form') }}" data-bs-toggle="tooltip"
                                ...
                                @endforeach
                            @else
                                <h4 class="text-center">No Performance Review!</h4>
                            @endif
                        </div>
                    </div>
                    {{-- <div class="card">
